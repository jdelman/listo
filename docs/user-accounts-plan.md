...
Add username/password login, a profile page, password recovery, and private data ownership. Bootstrap `jdelman` using the initial password supplied in the task through a one-time secret input, storing only its salted password hash. Allow changing that password later without forcing a first-login change. Start with operator-created accounts; public signup, sharing, and social login are separate features.
...
- `/profile`: current username, password change requiring the current password, and logout. Keep username read-only initially so it remains a stable login identity.
...
- Bind local MCP to an explicitly configured account resolved on startup; fail closed if missing or invalid. Document that stdio MCP is a trusted local operator integration. Any future remote MCP requires its own authenticated user identity.
...
## 5. Rollout and completion

Implement in dependency order: migration and account bootstrap; authenticated backend and worker/MCP changes; pages and client state; integration verification; deployment and data migration. Ship the ownership changes and authentication together.
...
Commit and push implementation changes as required by AGENTS.md. The initial password must not be hard-coded in source or committed documentation. Completion requires the deployed database migration and login smoke test, not just successful code changes.
