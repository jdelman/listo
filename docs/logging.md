...
A subsequent restart at `2026-09-10T21:15:12Z` recorded `worker.fatal`: `Cannot read properties of undefined (reading 'loadEnvConfig')`. The dynamic default import of `@next/env` failed under the `tsx` CLI. Switching to a static default import fixed the reproduced startup failure. CLI regression tests now launch against isolated empty databases, verifying both environment loading and a clear missing-key error.
