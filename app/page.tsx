...
function ListScreen({ store, listId, setScreen, flash }: { store: ReturnType<typeof useListoStore>; listId: string; setScreen: (screen: Screen) => void; flash: (message: string) => void }) {
  ...
  return <section>
    ...
    {editing ? <form className="panel form-grid" onSubmit={saveList}>
      ...
      <label>Tags<input name="tags" defaultValue={list.tags.join(", ")} /></label><label>View<select name="view" defaultValue={list.defaultView}>{["list", "compact", "gallery", "playlist", "document", "table"].map((mode) => <option key={mode}>{mode}</option>)}</select></label>
...
function QuickAdd({ store, afterSave }: { store: ReturnType<typeof useListoStore>; afterSave: (listId: string) => void }) {
  ...
  return <section className="narrow"><div className="page-heading"><div><h1>Quick Add</h1><p>Save first. Organize when you want to.</p></div></div>
    ...
    {kind === "clothing" && <form className="panel form-grid" onSubmit={saveClothing}>
      <label>Name<input name="name" required /></label><label>Size <small>(freeform)</small><input name="size" placeholder="40R" /></label><label>Photograph<input name="photo" type="file" accept="image/*" /></label><label>Tags<input name="tags" placeholder="herringbone, vintage" /></label><label className="full">Notes<textarea name="description" rows={3} /></label>
    ...
    {kind === "movie" && <form className="panel form-grid" onSubmit={saveMovie}><label>Title<input name="title" required /></label><label>Year<input name="year" inputMode="numeric" /></label><label>TMDB ID <small>(optional)</small><input name="tmdbId" inputMode="numeric" /></label><label>Tags<input name="tags" /></label><label className="full">Notes<textarea name="description" rows={3} /></label><p className="full"><a href="https://www.themoviedb.org/search/movie" target="_blank" rel="noreferrer">Search TMDB for the ID ↗</a></p><button className="primary">Save movie</button></form>}
  </section>;
}
